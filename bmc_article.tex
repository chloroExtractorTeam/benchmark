...
They harbor their own ribosomes and a circular DNA genome with a size between \SIrange{120}{160}{\kilo\basepair} \cite{palmer_1985}\todo{diese range umfasst die meisten aber nicht alle Chloroplasten}.
...
Databases containing short read data for species where no reference chloroplast sequence is publicly available, exist. \todo{cite? SRA?}
...
Here, a $k$-mer analysis can be used to extract the most frequent reads. An example for this is implemented in \ce{} \cite{j_ankenbrand_chloroextractor:_2018}. \todo{ce ist hier der einzig explizit genannte Assembler, das wirkt wie bias :) entfernen?}
...
%\ak{etwas umständlich formuliert, hoffe ich hab es jetzt richtig formuliert und verstanden}
...
%\ak{in 'Our Setup' before, schreiben wir 10GiB und 4 threads, müsste dann vielleicht geändert werden}
...
Except for \fp{} all the other tools had at least one assembly that was unsuccessful in one run, but produced an output in the other.\ak{oben steht \go{} war als einziges immer gut, dann kommt except for \fp{} ?? does not seem to be consistent}
...
Both \fp{} and \np{} tend to have minor changes in the assembly when the overall performance is comparably well, leading to the arrow-shaped scatter plots.\ak{link to figure scatter plots is missing}
...
Software authors should constantly improve the usability\todo{ich bin nicht sicher ob ich das so hart sagen würde}, and be open to suggestions from the users.
...
\ak{and I think we should mention Analife as competing interest, normally people list all non-academic shareholder whatever stuff, just to be sure}\todo{Ich weiß nicht ob das so passt}
...
\begin{figure}[h!]
  ...
  \caption{\csentence{Scores between two repeated runs for reproducibility testing}
