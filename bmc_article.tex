...
This means: no data prep, no specific reference (but \plant{At} if required), default parameters, no manual finishing.
We further restricted our benchmark to paired Illumina datasets as these are the most common \todo{cite?} and most \todo{(all?)} tools in this study can not handle any other types.
...
\section*{Methods}
Source code for all methods is available at \url{https://github.com/chloroExtractorTeam/benchmark} and archived under \todo{(zenodo doi)}.
...
datasets at \todo{(zenodo links for simulated and real)}.
This study adheres to the guidelines for computational method benchmarking \cite{weber_essential_2018}.

\subsection*{Tool Selection}

We included tools designed for assembling chloroplasts from whole genome paired end Illumina sequencing data. The tools must be available as open source software and allow execution via a command line interface \todo{das noch oben in der scope section erwähnen}. A graphical user interface is not suitable for
...
\texttt{Organelle PBA} is designed for PacBio data and does not work with paired Illumina data only. \texttt{Genious} software \todo{cite - die empfohlene Ref passt nicht da wir Geneious ja gar nicht verwenden, wir bräuchten eine ref die sagt, dass es das protocol gibt @niklas. \url{https://support.geneious.com/hc/en-us/articles/227534768-How-do-I-cite-Geneious-in-a-paper-}} is closed source and GUI based. \texttt{sestaton/Chloro} (\url{https://github.com/sestaton/Chloro}) fits all criteria but is flagged as a work in progress and development seems to have ended two years ago.
...
\texttt{MitoBim} is specifically designed for mitochondrial genomes. Even though there is a claim by the author that it can be used for chloroplasts as well (\url{https://github.com/chrishah/MITObim/issues/16}, accessed: 2019-05-27), there is no further description on how to do that. \todo{haben wir es versucht?}
\todo{method description paper - war das method description paper nicht das chloroplast assembly protocol? @niklas}
...
Therefore we used a sliding window approach with seqkit \cite{seqkit} \todo{seqkit parameter - würde ich hier nicht erwähnen, seqkit wurde mehrfach mit verschiedenen sub commands und parametern aufgerufen, die genaue Doku ist hier: \url{https://github.com/chloroExtractorTeam/benchmark/blob/master/03_representative_datasets.md}}.
