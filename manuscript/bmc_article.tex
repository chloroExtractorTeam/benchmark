...
\author[
   addressref={aff1,aff2,aff5},                   % id's of addresses, e.g. {aff1,aff2}
...
Plastid genomes accommodate essential genes and are regularly utilized in biotechnology or phylogenetics.
%Two to three sentences of more detailed background, comprehensible to scientists in related disciplines.
Different assemblers that are able to asses the plastid genome, have been developed.
These assemblers often use data of whole genome sequencing experiments, which usually contain reads from the complete chloroplast genome as well, as input.

%One sentence clearly stating the general problem being addressed by this particular study.
The performance of different assembly tools has never been systematically compared. %to each other concerning success rate or computational requirements.
%One sentence summarising the main result (with the words “here we show” or their equivalent).
Here we present a benchmark of seven chloroplast assembly tools, capable to succeed in more than \SI{60}{\percent} of known real data sets.
...
The examination of more than \num{100} data sets with unknown plastid genomes led to the assembly of novel chloroplast genomes in almost \SI{50}{\percent} of the cases.
%\todo{on the novel set we only have 49/105 which is roughly 47 percent}
%One or two sentences to put the results into a more general context. 
We created docker images for each tested tool that are freely available for the scientific community and ensure reproducibility of the analyses.
These containers allow to analyze and screen data sets for chloroplast genomes using standard computational infrastructure.
...
%long single copy (\gls{lsc}) and a short single copy (\gls{ssc}) region
...
%\todo{gibt es dazu ein beispiel in unserem set? (niklas)}
...
%\todo{oben steht nicht alle? (niklas)}
% Frank: Hier geht es um Memory und CPU-Usage... Oben nur um Runtime
...
%\todo{hier ein halbsatz, was im score berücksichtigt wird? (niklas)}
%\todo{Jan: are these values correct? is table 3 up to date? This whole section might need to be rewritten, Yes all the tables and plots are up to date and to my knowledge correct}
...
%\todo{Frank or Jan: whoever wrote this sentence, what does it mean? \ce{} produced completely unsuccessful assemblies but consistently. Please re-write or remove! Dont think it was me}
...
For these assemblers the scores were virtually identical if both runs were succesful. %\todo{Jan: identical? basically identical}.
...
%\todo{Markus/Frank: YES should we host a specific page on the alitv page for all the novels? Here: \url{https://alitvteam.github.io/AliTV/chloroplast_benchmark/})}%
...
%\todo{FRANK: more info about the taxonomic distribution of our novels and how we extended the coverage of the tol}.  %% STILL 2 DO
...
%\todo{FRANK: Add discussion of novel}  %% STILL 2 DO
% Yes, what have we leraned form the novel chloroplasts .. e.g. gene content 
...
%\todo{Frank: where do the 60 percent come from?, is it not more like 40?}
...
%\todo{Markus: mito ist richtig, oder?}
% Frank nach Rücksprache mit Markus: Ja... Ist Mitochondrium!
...
%\todo{Frank: Nochmal was zum Baum schreiben und novels dort mit aufnehmen!}  %% STILL 2 DO
...
%\todo{Frank: Beschreibung des Submission prozesses und der Datenvorbereitung}
%to the closest relative as well as the last common ancestor have been calculated \todo{see table X (Frank)}. \todo{further description of the method (Frank)}
...
%\todo{Frank: should we add some supplementary figure from plots/novel\_cutoffs.pdf?}
% Frank: Nicht notwendig

\subsubsection*{Consistency}
To ensure consistency of the obtained results, we rerun and re-evaluated all the assemblies. %\todo{JAN: Das sind jetzt aber alle, oder? Comment: Ja das sind alle }
...
\begin{figure}[h!]
  ...
  \caption{\csentence{Supplemental Figure. Number of distinct features in novel chloroplast genomes} The distribution of feature types tRNA, rRNA, and coding sequence (CDS) are shown separately.%
      }
...
\begin{table}[h!]
\caption{\textbf{Docker images used in our benchmark setup} SHA256 checksums are stated in \cref{tab:dockerimages_suppl}}
%\todo{Frank: add software version column (maybe use git describe)} %% STILL 2 DO
\label{tab:dockerimages}
\centering
      \resizebox{\textwidth}{!}{\begin{tabular}{ccc}
        \hline
          Tool & Image name and tag\\ \hline
          \ce & \dockerce \\
          \cassp & \dockercassp \\
          \fp & \dockerfp \\
          \go & \dockergo \\
...
\begin{table}[h!]
\caption{\textbf{Supplemenatal Table. Number of distinct features in novel chloroplast genomes} The distribution (mean, standard deviation (SD), medain, interquartile range (IQR)) of feature types tRNA, rRNA, and coding sequence (CDS) are listed separately.}
