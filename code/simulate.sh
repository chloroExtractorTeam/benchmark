...
seqkit sliding --window $SEGMENT --step 1 --threads 8 ${CIRCULAR} TAIR10_chr${CHR}.fas | seqkit sample --threads 8 --proportion $PROPORTION --rand-seed $SEED | seqkit shuffle --threads 8 --rand-seed $SEED | seqkit split --by-part 2 --threads 8 -O chr${CHR}_${READLEN}_intermediate
...
seqkit shuffle --threads 8 --rand-seed $SEED --out-file chr${CHR}_${READLEN}_intermediate2.fa.gz chr${CHR}_${READLEN}_intermediate/stdin.part_002.fasta
